  ...
  while (inputPosition < inputLength) {
    switch (ctx->state) {
      ...
      case LZ4_STATE_LITERAL_VALUE:
        ...
        writeFunction(&data[inputPosition], length);
    ...
    if (ctx->state == LZ4_STATE_BACKTRACKING) {
      ...
      while (ctx->matchLength > 0) {
        ...
        writeFunction(backtrackBuffer, backtrackBytes);
...
int32_t gbl_lz4WriteMemory(uint8_t *data, size_t length)
{
  ...
  // We have some aligned data, write it out
  if (alignedLength) {
    gbl_writeProgData(lz4ParserContext.parserContext,
                      &data[offset],
                      alignedLength,
