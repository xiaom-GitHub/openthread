...
static void     StartTransferDMAB(SPIDRV_Handle_t handle,
                                 const void *txBuffer,
                                 void *rxBuffer,
                                 int txCount,
                                 int rxCount,
                          ...
                          int timeoutMs)
{
  ...
  if ( (retVal = TransferApiBlockingPrologue(handle, (void*)txBuffer, txCount))
       != ECODE_EMDRV_SPIDRV_OK ) {
    return retVal;
  }
  ...
  //StartTransferDMA(handle, txBuffer, rxBuffer, txCount, rxCount, BlockingComplete);

  StartTransferDMAB(handle, txBuffer, rxBuffer, txCount, rxCount, BlockingComplete);
  WaitForTransferCompletion(handle);
