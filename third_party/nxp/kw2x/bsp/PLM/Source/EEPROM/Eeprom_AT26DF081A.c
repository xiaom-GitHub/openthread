...
#if defined(MCU_MK60D10) || defined(MCU_MK60N512VMD100)
  #define RADIO_ProtectFromInterrupt(var) {var = MC1324xDrv_IRQ_IsEnabled(); MC1324xDrv_IRQ_Disable();}
  #define RADIO_UnprotectFromInterrupt(var)  if (var) MC1324xDrv_IRQ_Enable();
#else
  #define RADIO_ProtectFromInterrupt(var)   var = 0;
  #define RADIO_UnprotectFromInterrupt(var) (void)var;
#endif
...
ee_err_t EEPROM_EraseBlock(uint32_t Addr, uint32_t size)
{
  ...
  RADIO_ProtectFromInterrupt(radioIrqSts);
  ...
  RADIO_UnprotectFromInterrupt(radioIrqSts);
