...
ztcIBData_t const maZtcIBData[] = {
  /* Network Information Base Identifiers. 053474r20 ZigBee Pro spec - Table 3.44 NWK IB Attributes. */
  ...
  {gNwkLeaveRequestAllowed_c,               mZtcIBRW, sizeof(NlmeGetRequest(gNwkLeaveRequestAllowed_c)),    mIBScalar,                              (void *) &NlmeGetRequest(gNwkLeaveRequestAllowed_c)},

/* 
  053474r20 ZigBee Pro spec Zigbee Spec.
...
*/
  ...
  {gNwkProtocolVersion_c,                   mZtcIBRW, sizeof(gSAS_Ram.protocolVersion),              mIBScalar,                              (void *) &gSAS_Ram.protocolVersion},
  ...
  {gNwkLinkRetryThreshold_c,                mZtcIBRW, sizeof(NlmeGetRequest(gNwkLinkRetryThreshold_c)),   mIBScalar,                              (void *) &NlmeGetRequest(gNwkLinkRetryThreshold_c)},
  ...
  {gNwkPreconfiguredKey_c,                  mZtcIBRW, sizeof(NlmeGetRequest(gNwkPreconfiguredKey_c)),          mIBScalar,           (void *) &NlmeGetRequest(gNwkPreconfiguredKey_c)},
  {gNwkState_c,                             mZtcIBRO, sizeof(NlmeGetRequest(gNwkState_c)),                    mIBScalar,           (void *) &NlmeGetRequest(gNwkState_c)},
  ...
  /* APS Information Base Identifiers. 053474r20 ZigBee Pro spec - Table 2.24 */
...
#define pRequestHdr (gZtcPacketFromClient.structured.header)
  ...
  for (i = 0, pIBData = NULL; i < NumberOfElements(maZtcIBData); ++i) {
    ...
    {
      /*
        For the apsDeviceKeyPairSet AIB the first entry in maZtcIBData[] will be
        that of the nwkLeaveRequestAllowed NIB. This is because the
        nwkLeaveRequestAllowed NIB and apsDeviceKeyPairSet AIB share the same id.
        Because of this the first matching entry in the maZtcIBData[] will be
        skipped for the apsDeviceKeyPairSet AIB.
      */
      if((pRequestHdr.msgType == mZtcMsgApsmeGetIBReq_c)
         && (pRequest->iId == gApsDeviceKeyPairSet_c)
           && skip)
      {
        skip = FALSE;
        continue;
...
#define pRequestHdr (gZtcPacketFromClient.structured.header)
  ...
  for (i = 0, pIBData = NULL; i < NumberOfElements(maZtcIBData); ++i) {
    if (pRequest->iId == maZtcIBData[i].id)
    {
      /*
        ...
        skipped for the apsDeviceKeyPairSet AIB.
      */
      if((pRequestHdr.msgType == mZtcMsgApsmeSetIBReq_c)
         && (pRequest->iId == gApsDeviceKeyPairSet_c)
           && skip)
      {
        skip = FALSE;
  ...
  if (pRequest->iId == gNwkPanId_c)
    (void)SetPibAttributeValue(gMPibPanId_c, (void *)&pRequest->pData);
  else if (pRequest->iId == gNwkShortAddress_c)
