...
bool_t ZtcComm_BufferIsEmpty(void)
{
  ...
  return (0 == IIC_GetRxBytesCount());
#elif gCDCInterface_d
  return (0 == gCDCRxBufferByteCount);
#elif gSPIInterface_d
  return (0 == SPI_GetSlaveRecvBytesCount());  
#endif  
}
...
#elif gI2CInterface_d
  IIC_SetSlaveRxCallBack(ZtcComm_RxCallback);      
#elif gCDCInterface_d
  CDC_SetRxCallBack(ZtcComm_RxCallback);
#elif gSPIInterface_d  
  SPI_SetSlaveRxCallBack(ZtcComm_RxCallback);
#endif
}
                       ...
                       )
{
...
#if gZtcNonBlockingFunctionality_c   
   pMsg = MSG_Alloc(length + 2); /* STX, data, checksum */
  ...
  else
  {
    ...
    pMsg[length + 1]  = checksum;
  
    /* Send frame */
#if  gSPIInterface_d   
  while (!transmit(pMsg, length + 2,(void (*)(unsigned char *))MM_Free)){}
#elif gCDCInterface_d
  if(!transmit(pMsg, length + 2,(void (*)(unsigned char const *))MM_Free))
  {
    MM_Free(pMsg);
    gZtcCommTxBlocking = FALSE;
  }
#else
