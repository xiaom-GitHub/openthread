  ...
  )
{
 ...
 /*  *((uint8_t **)(pMsg + hdrFirstPartLen)) = (pMsg + offsetToMsgArray1); */
  ...
  /* second array. */
  if (offsetToMsgPtr2 && fmtInfo.sizeofArray2Element) {
...
{
  uint16_t arrayLen;                     /* Risky on an 8 bit CPU. */
  index_t code;
  ztcFmtLenCodeAndPtr_t fmtInfo;
  index_t elementLen;
  index_t hdrTotalLen;
  ...
  if (hdrFirstPartLen == (index_t) gMinusOne_c) {
    ...
    /* with the Test Tool. */
    FLib_MemCpy((pPkt + hdrFirstPartLen), &arrayLen, sizeof(uint16_t));
    /* Copy the second part of the header. */
...
index_t ZtcMsgFromPktNlmeEnergyScanReq
(
  uint8_t *pMsg,
  index_t msgLen,
  uint8_t *pPkt,
...
{
  nlmeEnergyScanReq_t *pPktScanReq;
  index_t result;
  pPktScanReq = (nlmeEnergyScanReq_t *)pPkt;
  
  /* Saving channel list in MAC Scan Request */
  maScanChannels[ 0 ] = pPktScanReq->aScanChannels[ 0 ];
  maScanChannels[ 1 ] = pPktScanReq->aScanChannels[ 1 ];
  maScanChannels[ 2 ] = pPktScanReq->aScanChannels[ 2 ];
  maScanChannels[ 3 ] = pPktScanReq->aScanChannels[ 3 ];
...
index_t ZtcPktFromMsgNlmeEnergyScanReq
(
  uint8_t *pMsg,
  index_t msgLen,
  uint8_t *pPkt,
...
/****************************************************************************/
/* Custom function for copying ASP XCVR Read message into the ZTC packet.
 */
#if gSAPMessagesEnableAsp_d
index_t ZtcPktFromMsgAspXcvrRead
(
  uint8_t *pPkt,
  index_t pktLen,
  uint8_t *pMsg,
...
{
    aspXcvrReq_t *msg = (aspXcvrReq_t*)pMsg;

    *pPkt++ = msg->len;
    FLib_MemCpy(pPkt, (uint8_t*)&msg->data, msg->len);
