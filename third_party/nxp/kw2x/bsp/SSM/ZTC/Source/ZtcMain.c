...
#if gZtcIncluded_d && gCDCInterface_d
...
typedef PACKED_STRUCT apsdeFragment_tag {
...
#if gBeeStackIncluded_d && gZTC_RegEpCapability && gCDCInterface_d
static void ResetRegisteredEndPoints(void);
#endif
...
#if gCDCInterface_d
static void ZtcCdcExtendedTablesReset(void);
#endif
...
#if gZtcIncluded_d && gNvStorageIncluded_d
	...
	if (maZtcSAPModeTable[pSrcSAPInfo->modeIndex] == gZtcSAPModeHook_c)
	{
		if (pSrcSAPInfo->pMsgQueue)
		{
			...
			if (pSrcSAPInfo->opcodeGroup == gMlmeNwkOpcodeGroup_c) /* If the message came from MAC layer */
			{
				...
				{
				    if (pMsg->msgData.scanCnf.scanType == gScanModeED_c || pMsg->msgData.scanCnf.scanType == gScanModeFastED_c)
                                    {
                                      MSG_Free(pMsg->msgData.scanCnf.resList.pEnergyDetectList);
                                      pMsg->msgData.scanCnf.resList.pEnergyDetectList = NULL;
                                    }
                                    else
                                    {
	                              if (pMsg->msgData.scanCnf.resList.pPanDescriptorBlocks->descriptorList) /* If the internal list of the scan has somem data or point to some place */
                                      {/* Free the internal list comming in the the Scan from the MAC (second block) */
                                        while(pMsg->msgData.scanCnf.resList.pPanDescriptorBlocks)
                                        {
				          panDescriptorBlock_t* pNext = pMsg->msgData.scanCnf.resList.pPanDescriptorBlocks->pNext;
					  ...
					  MSG_Free(pMsg->msgData.scanCnf.resList.pPanDescriptorBlocks);
                                          pMsg->msgData.scanCnf.resList.pPanDescriptorBlocks = pNext;
                                        }
                                      }
                                    }
                                }
...
{
  gOverrideApsFrameEncryptionSettings_c.overrideUpdateDeviceEncryption = 
    gZtcPacketFromClient.structured.payload[0];
  gOverrideApsFrameEncryptionSettings_c.updateDeviceEncryption = 
    gZtcPacketFromClient.structured.payload[1];
  gOverrideApsFrameEncryptionSettings_c.overrideSecUpdateDeviceProcessing = 
    gZtcPacketFromClient.structured.payload[2];
...
void ZtcMsgSetApsDeviceKeyPairSetKeyInfo(void)
{
  ...
  size_t index = gZtcPacketFromClient.structured.payload[0];
  if(index >= giApsDeviceKeyPairCount) 
    status = gZbFailed_c;
  else
  {
    gaApsDeviceKeyPairSet[index].iKey = gZtcPacketFromClient.structured.payload[0];
    gaApsDeviceKeyPairSet[index].keyType = gZtcPacketFromClient.structured.payload[1];    
  }
  ZtcMsgSetCnfStatus(status);
#endif
}
...
#endif
    {
      uint16_t firstByte = gZtcPacketFromClient.structured.payload[0];
      uint16_t secondByte = gZtcPacketFromClient.structured.payload[1];
      firstByte = firstByte | (secondByte << 8);
      gZdoOrgPollRate = firstByte;
      NlmeSetRequest(gNwkIndirectPollRate_c, gZdoOrgPollRate);
...
{
  if (gZtcPacketFromClient.structured.header.len >= sizeof(gRejoinConfigParams))
    FLib_MemCpy(&gRejoinConfigParams, gZtcPacketFromClient.structured.payload, sizeof(gRejoinConfigParams));
...
void ZtcMsgSetApsDeviceKey(void)
{
  ...
  if(!KeySet_SetTableEntry(gZtcPacketFromClient.structured.payload[0], &gZtcPacketFromClient.structured.payload[1]))
    status = gZbFailed_c;  
...
void ZtcMsgGetApsDeviceKey(void)
{
  ...
  if(!KeySet_GetTableEntry(gZtcPacketFromClient.structured.payload[0], &gZtcPacketToClient.structured.payload[1]))
    status = gZbFailed_c;  
...
#else
  uint8_t* pDataSetCount;
  uint16_t dataSetIndex;
  
  gZtcPacketToClient.structured.payload[payloadIndex++] = 0;
  /* left for nvm table count */  
  pDataSetCount = &gZtcPacketToClient.structured.payload[payloadIndex++]; 
 
  
  *pDataSetCount=0;
  
  dataSetIndex = gZtcPacketFromClient.structured.payload[1];
  dataSetIndex <<= 8;
  dataSetIndex+= gZtcPacketFromClient.structured.payload[0];
  
  for ( ;dataSetIndex < gNvTableEntriesCountMax_c; dataSetIndex++) 
  {
    if(gNvInvalidDataEntry_c != (pNVM_DataTable+dataSetIndex)->DataEntryID)
    {
      if(NULL != (pNVM_DataTable+dataSetIndex)->pData)
      {
         uint32_t address = (uint32_t)(pNVM_DataTable+dataSetIndex)->pData; 
         /* table DataEntryID, ElementSize,ElementsCount are max 255 */
         FLib_MemCpy(&gZtcPacketToClient.structured.payload[payloadIndex],&address,4);
         payloadIndex+=4;  
         gZtcPacketToClient.structured.payload[payloadIndex++] = ((pNVM_DataTable+dataSetIndex)->DataEntryID & 0xFF);
         gZtcPacketToClient.structured.payload[payloadIndex++] = ((pNVM_DataTable+dataSetIndex)->ElementSize & 0xFF);
         gZtcPacketToClient.structured.payload[payloadIndex++] = ((pNVM_DataTable+dataSetIndex)->ElementsCount & 0xFF);
         (*pDataSetCount)++;
         if(gZtcPacketFromClient.structured.payload[2])
         {
           break;
         }
        }
        else if(gZtcPacketFromClient.structured.payload[2])
        {
          break;
        }
      }
    ...
    /* protection for nvm "extreme size" tables, code shall be updated - not necessary for now*/  
    if(payloadIndex > 155) break;
  }
#endif  
  gZtcPacketToClient.structured.header.len = payloadIndex;
...
#if gZtcIncluded_d
void ZtcMsgGetNVCountersReqFunc(void) {
  
#if gNvStorageIncluded_d 
  index_t payloadIndex = sizeof(clientPacketStatus_t);
  NVM_Statistics_t ptrStat;
  gZtcPacketToClient.structured.payload[0] = 0; 
  NvGetPagesStatistics(&ptrStat);

  FLib_MemCpy(&gZtcPacketToClient.structured.payload[payloadIndex],&ptrStat.FirstPageEraseCyclesCount,4);
  payloadIndex+=4;
  FLib_MemCpy(&gZtcPacketToClient.structured.payload[payloadIndex],&ptrStat.SecondPageEraseCyclesCount,4);
  payloadIndex+=4;
  gZtcPacketToClient.structured.header.len = payloadIndex;
#else
 ZtcError(gZtcRequestIsDisabled_c);
#endif
} /* ZtcMsgGetNVCountersReqFunc() */
#endif

/****************************************************************************/
#if gZtcIncluded_d 
void ZtcMsgSetNVMonitoring(void)
{
  #if !gNvStorageIncluded_d || !gZtcNvmMonitoringEnable_c
  ZtcError(gZtcRequestIsDisabled_c);
  #else
  nvmMonitorOn = !!gZtcPacketFromClient.structured.payload[0];
  gZtcPacketToClient.structured.header.len         = sizeof(clientPacketStatus_t)+1;
  gZtcPacketToClient.structured.payload[0] = 0; 
  #endif
}
#endif

#if gZtcIncluded_d && gNvStorageIncluded_d && gZtcNvmMonitoringEnable_c
/****************************************************************************/
void ZtcMsgNVPageEraseMonitoring(uint32_t pageAddress, uint8_t status)
{
  
  if (nvmMonitorOn)
  {
    gZtcPacketToClient.structured.header.opcodeGroup = gZtcCnfOpcodeGroup_c;
    gZtcPacketToClient.structured.header.msgType     = mZtcMsgNVPageEraseMonitoring_c;
    
    FLib_MemCpy(&gZtcPacketToClient.structured.payload[0],&pageAddress ,4);
    
    gZtcPacketToClient.structured.payload[4]=status;
    gZtcPacketToClient.structured.header.len = 5;
...
void ZtcMsgNVWriteMonitoring(uint16_t nvmTableEntryId, uint16_t elementIndex, uint8_t saveRestoreAll)
{
  
  if (nvmMonitorOn)
  {
    gZtcPacketToClient.structured.header.opcodeGroup = gZtcCnfOpcodeGroup_c;
    gZtcPacketToClient.structured.header.msgType     = mZtcMsgNVWriteMonitoring_c;
    
    FLib_MemCpy(&gZtcPacketToClient.structured.payload[0],&nvmTableEntryId ,2);
    FLib_MemCpy(&gZtcPacketToClient.structured.payload[2],&elementIndex ,2);
    gZtcPacketToClient.structured.payload[4]=saveRestoreAll;
    gZtcPacketToClient.structured.header.len = 5;
...
void ZtcMsgNVRestoreMonitoring(uint16_t nvmTableEntryId, bool_t bStart, uint8_t status)
{
  
  if (nvmMonitorOn)
  {
    gZtcPacketToClient.structured.header.opcodeGroup = gZtcCnfOpcodeGroup_c;
    gZtcPacketToClient.structured.header.msgType     = mZtcMsgRestoreMonitoring_c;
    
    FLib_MemCpy(&gZtcPacketToClient.structured.payload[0],&nvmTableEntryId ,2);
    
    gZtcPacketToClient.structured.payload[2]=bStart;
    gZtcPacketToClient.structured.payload[3]=status;
...
void ZtcMsgNVVirtualPageMonitoring(bool_t bStart, uint8_t status)
{
  
  if (nvmMonitorOn)
  {
    gZtcPacketToClient.structured.header.opcodeGroup = gZtcCnfOpcodeGroup_c;
    gZtcPacketToClient.structured.header.msgType     = mZtcMsgVirtualPageMonitoring_c;
        
    gZtcPacketToClient.structured.payload[0]=bStart;
    gZtcPacketToClient.structured.payload[1]=status;
...
void ZtcMsgNVSaveReqFunc(void) 
{
  ...
  {
    /* data set not found */
    gZtcPacketToClient.structured.payload[0] = gNVM_InvalidTableEntry_c; 
    return;
  }
  
  if(!gZtcPacketFromClient.structured.payload[5])
  {
    dataSetElement = gZtcPacketFromClient.structured.payload[2];
    dataSetElement <<= 8;
    dataSetElement += gZtcPacketFromClient.structured.payload[3];
  }
  
  
  switch(gZtcPacketFromClient.structured.payload[4])
  {
    case 0:
      gZtcPacketToClient.structured.payload[0] = NvSaveOnIdle((uint8_t*)(pNVM_DataTableEntry->pData)+
                              dataSetElement*pNVM_DataTableEntry->ElementSize,
                              !!gZtcPacketFromClient.structured.payload[5]);
      break;
    case 1:
      gZtcPacketToClient.structured.payload[0] = NvSaveOnInterval(pNVM_DataTableEntry->pData);
      break;
    case 2:
      gZtcPacketToClient.structured.payload[0] = NvSaveOnCount(pNVM_DataTableEntry->pData);
      break;
    case 3:
      if(!!gZtcPacketFromClient.structured.payload[6])
      {
        gZtcPacketToClient.structured.payload[0] = NvAtomicSave(TRUE);
      }
      else
      {
        gZtcPacketToClient.structured.payload[0] = 
      ...
      break;
    default:
      gZtcPacketToClient.structured.payload[0] = gNVM_SaveRequestRejected_c; 
      return;
  }  
#endif
}                                       /* ZtcMsgNVSaveReqFunc() */
...
/* Format nvm pages */
#if gZtcIncluded_d 
void ZtcMsgNVFormatReq(void)
{
  
  #if !gNvStorageIncluded_d
  ZtcError(gZtcRequestIsDisabled_c);
  #else
  gZtcPacketToClient.structured.payload[0] = NvFormat();
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t)+1;
  #endif
}
#endif
...
void ZtcMsgNVRestoreReq(void)
{
  ...
  dataSetIndex = gZtcPacketFromClient.structured.payload[3];
  dataSetIndex <<= 8;
  dataSetIndex+= gZtcPacketFromClient.structured.payload[2];
  ...
  while (gNvEndOfTableId_c != (pNVM_DataTable+countInd)->DataEntryID)
  {
    ...
    {    
       ...
       gZtcPacketToClient.structured.payload[0] = 
                      NvRestoreDataSet(pData,!!gZtcPacketFromClient.structured.payload[1]);
       if((!gZtcPacketFromClient.structured.payload[0])||(gZtcPacketToClient.structured.payload[0]))
       {
          break;
       }
...
static uint8_t mNextPushChunkSeq = 0;
...
void ZtcOtaSupportSetModeFunc(void) 
{
  ...
  if(mOtaSetMode > gOtaTestingProcess)
  {
...
#if gUpgradeImageOnCurrentDevice_d      
      /* Add seq number */
      gZtcPacketToClient.structured.payload[1] = gZtcPacketFromClient.structured.payload[0];

      /* Check the seq number of the chunk. Length - 1B for chunk seq num */
      if (gZtcPacketToClient.structured.payload[1] == mNextPushChunkSeq)
      {
        gZtcPacketToClient.structured.payload[0] = OTA_PushImageChunk(&gZtcPacketFromClient.structured.payload[1],len - 1, NULL); 
        
        if (!gZtcPacketToClient.structured.payload[0])
          mNextPushChunkSeq++;
      }
      else
      {
        /* Chunk was previously written successfully*/
        gZtcPacketToClient.structured.payload[0] = gOtaSucess_c;
      }      
#else
        gZtcPacketToClient.structured.payload[0] = gOtaInvalidParam_c;      
...
#if gUpgradeImageOnCurrentDevice_d	
    /* Add one byte for chunk seq number */
    gZtcPacketToClient.structured.header.len += (sizeof(uint8_t));
#endif	
}
...
void ZtcOtaSupportCommitImageReqFunc(void) 
{
    ...
    if(mOtaSetMode  == gUpgradeImageOnCurrentDevice_c)
    {
        ...
        gZtcPacketToClient.structured.payload[0] = OTA_CommitImage(gZtcPacketFromClient.structured.payload[0], 
                                                                OTA2Native32(bitmap));  
#else
         gZtcPacketToClient.structured.payload[0] = OTA_CommitImage(gZtcPacketFromClient.structured.payload);   
#endif         
        OTA_SetNewImageFlag();
#else
        gZtcPacketToClient.structured.payload[0] = gOtaInvalidParam_c;    
...
void ZtcOtaSupportCancelImageReqFunc(void) 
{
   ...
   mLoadInProgres = FALSE;
   gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));  
   gZtcPacketToClient.structured.payload[0] =  OTAServer_CloseOTAProcess();
#endif   
}

/****************************************************************************
 * ZtcOtaSuportSetLocalFileVersionPoliciesFunc:
 *   - calling this function will set the local file version policies
 ****************************************************************************/
void ZtcOtaSuportSetLocalFileVersionPoliciesFunc(void) 
{
#if gZclEnableOTAServer_d   
   gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));  
   gZtcPacketToClient.structured.payload[0] =  OTAServer_SetLocalFileVersionPolicies(gZtcPacketFromClient.structured.payload[0]);
#endif   
}
 ...
 ****************************************************************************/
void ZtcOtaSupportAbortOTAUpgradeFunc(void) 
{
#if gZclEnableOTAServer_d   
   mLoadInProgres = FALSE;
   gZtcPacketToClient.structured.header.len = (sizeof(uint8_t));  
   gZtcPacketToClient.structured.payload[0] =  OTAServer_AddToAbortBlockRspList(&gZtcPacketFromClient.structured.payload[0]);
#endif   
}
...
void ZtcMsgResetCPUReqFunc(void) {
  ...
  ZtcCdcExtendedTablesReset();
#if gBeeStackIncluded_d
#if gZTC_RegEpCapability && gZtcIncluded_d
  ResetRegisteredEndPoints();
#endif                                  /* #if gZTC_RegEpCapability */
#endif /* gBeeStackIncluded_d */
#else  /* gCDCInterface_d*/
  ResetMCU();
#endif /* gCDCInterface_d*/
}
#endif
...
#endif
/****************************************************************************/
#if (gZtcIncluded_d && gZtcLpmSuport_d)
void Ztc_SendWakeUpIndication(uint8_t WakeupReason)
{
  while(isTxActive());
  gZtcPacketToClient.structured.header.opcodeGroup = gZtcCnfOpcodeGroup_c;
  gZtcPacketToClient.structured.header.msgType = mZtcMsgWakeUpIndication_c; 
  gZtcPacketToClient.structured.header.len = sizeof(uint8_t);
  gZtcPacketToClient.structured.payload[0]=WakeupReason;
  ZtcComm_WritePacketToClient(sizeof(gZtcPacketToClient.structured.header) + 
                              gZtcPacketToClient.structured.header.len);
}

void ZtcMsgAllowDeviceToSleep(void)
{
  gZtcPacketToClient.structured.header.opcodeGroup = gZtcCnfOpcodeGroup_c;
  gZtcPacketToClient.structured.header.msgType = mZtcMsgAllowDeviceToSleepReq_c; 
  gZtcPacketToClient.structured.header.len = sizeof(uint8_t);
#if gLpmIncluded_d  
  gZtcPacketToClient.structured.payload[0]=gZtcSuccess_c;
#else
  gZtcPacketToClient.structured.payload[0]=gZtcRequestIsDisabled_c;
#endif
  ...
  /* Short delay  for sending confirm to host */
  {
    volatile uint32_t delay=0x0001FFFF;
    while(delay--);
  } 
#if gLpmIncluded_d   
...
{
  gNwkHighRamConcentrator = gZtcPacketFromClient.structured.payload[0];
  gZtcPacketToClient.structured.payload[0] = gZtcSuccess_c;
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t); 
}
#endif
...
#if gBeeStackIncluded_d
#if gZTC_RegEpCapability && gZtcIncluded_d && gCDCInterface_d
static void ResetRegisteredEndPoints(void)
{
	uint8_t i, j;
	uint8_t status;
	...
	for(i=2+gNum_EndPoints_c; i<gNoOfEndPoints; i++)
	{
		if(gaEndPointDesc[i].pDescription)
		{
			pSimpleDescriptor = (zbSimpleDescriptor_t *)gaEndPointDesc[i].pDescription->pSimpleDesc;
			status = AF_DeRegisterEndPoint(pSimpleDescriptor->endPoint);
			if (status == gZbSuccess_c)
			{
				for (j = 0; j < NumberOfElements(maSimpleDescriptors); j++)
				{
					if (maSimpleDescriptors[j].endPoint == pSimpleDescriptor->endPoint)
					{
						maSimpleDescriptors[j].endPoint = 0;
						break;
					}
			    }
			}
...
#if (gReplace_Device_rsp_d || gRemove_Bkup_Bind_Entry_rsp_d || gBackup_Bind_Table_rsp_d || gStore_Bkup_Bind_Entry_rsp_d || gRecover_Bind_Table_rsp_d || gBind_Register_rsp_d || gMgmt_Bind_rsp_d)
	BeeUtilZeroMemory(gaBindingCacheTable, sizeof(zbApsmeBindEntry_t) * gMaximumBindingCacheTableList_c);
	gBindingCacheTableCounter = 0;
#endif
#if gBind_Register_rsp_d || gRecover_Source_Bind_rsp_d || gBackup_Source_Bind_rsp_d || (gBindCapability_d && gBind_rsp_d)
	BeeUtilZeroMemory(aDevicesHoldingItsOwnBindingInfo, sizeof(zbIeeeAddr_t) * gMaximumDevicesHoldingBindingInfo_c);
	gDeviceHoldingItsOwnBindingInfo_Counter = 0;
#endif
#if (gDiscovery_store_rsp_d || gRemove_node_cache_rsp_d || gMgmt_Cache_rsp_d)
	mDiscoveryStoreTableEntrys = 0;
#endif
#if gBkup_Discovery_cache_d || gDiscovery_store_rsp_d || gMgmt_Cache_rsp_d || gRemove_node_cache_rsp_d || gFind_node_cache_rsp_d|| gActive_EP_store_rsp_d || gPower_Desc_store_rsp_d || gNode_Desc_store_rsp_d || gSimple_Desc_store_rsp_d
	{
	  uint8_t i;
	  for(i=0; i<gMaximumDiscoveryStoreTableSize_c; i++)
	  {
		BeeUtilZeroMemory(&gaDiscoveryStoreTable[i], sizeof(discoveryStoreTable_t));
		//BeeUtilSetToF(gaDiscoveryStoreTable[i].aNwkAddress, sizeof(zbNwkAddr_t));
	  }
	}
#endif
	gaServerMask[0] = gServerMask_lEndianFirstByte_d;
	gaServerMask[1] = gServerMask_lEndianSecondByte_d;
#if gBind_req_d && gBind_Register_rsp_d
	BeeUtilZeroMemory(maEventProcessQueue, sizeof(zbEventProcessQueue_t) * gMaximumEventsToProcess_c);
#endif
}
#endif
...
{
  uint8_t ezMode = 	gZtcPacketFromClient.structured.payload[2];
  gEZCommissioningPrimaryDeviceType = gZtcPacketFromClient.structured.payload[0];
  gEZCommissioningUseGroup = gZtcPacketFromClient.structured.payload[1];
