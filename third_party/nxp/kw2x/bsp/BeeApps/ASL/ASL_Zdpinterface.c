  ...
  )
{
  ...
  if(event == gzdo2AppEventInd_c) 
  {
      ...
      BeeAppUpdateDevice(appEndPoint, appEvent, 0, NULL, NULL); 
      return;
  }
  ...
  if (event == gIEEE_addr_rsp_c ) 
  {
        if (MsgFromZDP->msgData.extAddressResp.status == gZbSuccess_c)
        {
              ...
              BeeAppUpdateDevice(appEndPoint, gIeeeAddrSuccess_c, 0, NULL, &MsgFromZDP->msgData.extAddressResp.devResponse.extendedDevResp);
              MSG_Free(MsgFromZDP);
              return;
  ...
  {  
        ...
        {
              BeeAppUpdateDevice(appEndPoint, gNwkAddrSuccess_c, 0, NULL, &MsgFromZDP->msgData.nwkAddressResp.devResponse.singleDevResp);
        }
        else
        {
              sendEventToApp = TRUE;
  ...
  {  
        ...
        {
              BeeAppUpdateDevice(appEndPoint, gNodeDescSuccess_c, 0, NULL, &MsgFromZDP->msgData.nodeDescriptorResp);
        }
        else
        {
              sendEventToApp = TRUE;
  ...
  {
#if gASL_EnableEZCommissioning_d    
        if(gEZCommissioning_LastEvent == gSendSimpleDescReq_c)
        {
            uint8_t length;
            ...
            FLib_MemCpy(gEZCommissioningTempData, &MsgFromZDP->msgData.simpleDescriptorResp, length);
  ...
  if (MsgFromZDP->msgType == gNlmeJoinConfirm_c)
  {
        if (MsgFromZDP->msgData.joinCnf.status != gZbSuccess_c)
        {
            if (ZDO_GetJoinMode() == gNwkRejoin_c)
              BeeAppUpdateDevice(appEndPoint, gRejoinFailed_c, 0, NULL, &MsgFromZDP->msgData.joinCnf);
        }
        else
        {
            sendEventToApp = TRUE;
