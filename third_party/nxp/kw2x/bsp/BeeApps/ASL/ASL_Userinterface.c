  ...
  )
{
  ...
  switch (event) {
...
#endif	//SmartEnergyApplication_d 
      }
    ...
    case gZclUI_EZCommissioning_Succesfull_c:  
      ...
      ASL_ConfigSetLed(LED4,gLedOff_c);   
#endif	  
      ASL_ConfigSetLed(LED3,gLedOn_c);
#if gZclEnableOTAClient_d  &&   gZclOTADiscoveryServerProcess_d   
      {
        zbClusterId_t clusterId = {gaZclClusterOTA_c};
    	zclInitiateOtaProcess_t initServerDiscovery = {0, 0};
...
{
#if gLCDSupported_d	
   uint32_t time = OTA2Native32(gZclTimeServerAttrs.LocalTime);
   uint32_t val;
   uint32_t daySec;
...
#if gASL_PrintToTestClient_d
    {
      uint8_t message[gMAX_LCD_CHARS_c];
      FLib_MemSet(message,  gMAX_LCD_CHARS_c, ' ');
      FLib_MemCpy(message, pMsg, 16);
      ASL_PrintToTestClient(message);
    }
#endif    
