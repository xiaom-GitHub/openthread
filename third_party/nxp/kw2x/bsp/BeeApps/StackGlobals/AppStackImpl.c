...
#if (gMgmt_NWK_Update_req_d &&(gNetworkManagerCapability_d || gComboDeviceCapability_d))
/*
  The timer to be used during to request the  Energy Scan Req to Routers. Used
  only by the Network Manager
*/
tmrTimerID_t gFA_ReqEnergyScanToRoutersTimer;
#endif
...
*/
static nwkAddrList_t *mpFA_ListOfRouters = NULL;
...
bool_t gSuitableParentFound = FALSE;

rejoinConfigParams_t gRejoinConfigParams = {0x00, 0x00, {0x00, 0x00}};
  ...
  */
  ...
  for (iIndex = 0; iIndex < gMaxDiscoveryTableEntries; iIndex++)
  {
    ...
    if ((panIdMatches) && ( NWK_DeviceIsPotentialParent(pEDTE->joiningFeatures) ) && ( linkCost <= maxLinkCost ))
    {
      ...
      {
        if ((((gRejoinConfigParams.paramFlags & gRejoinConfigParamsMask_EnableAddrFiltering) != gRejoinConfigParamsMask_EnableAddrFiltering)
             || !IsEqual2Bytes(pDTE->aNetworkAddress, gRejoinConfigParams.filterDeviceNwkAddress)) && 
            (((gRejoinConfigParams.paramFlags & gRejoinConfigParamsMask_EnableLqiFiltering) != gRejoinConfigParamsMask_EnableLqiFiltering)
             || pDTE->lqi >= gRejoinConfigParams.filterDeviceLqi))           
        {
...
#endif
            {
              if ((gRejoinConfigParams.paramFlags & gRejoinConfigParamsMask_EnablePreviousParent) == gRejoinConfigParamsMask_EnablePreviousParent)
              {
...
)
{
  ...
  else
  {
    ...
    FLib_MemCpy(gaScanChannelList, pChannelList, sizeof(uint32_t));
#else
...
#if (gMgmt_NWK_Update_req_d &&(gNetworkManagerCapability_d || gComboDeviceCapability_d))
  else if(tmrId == gFA_ReqEnergyScanToRoutersTimer)
  {
...
* (as indicated in the Zigbee spec 053474r20 ZigBee Pro spec- Annex E, for the
* purpose of giving the user a simple example of how and where this is done and it
* can be adjusted as need it).
*
* Interface assumptions:
...
)
{
  ...
  if (!ZdoGetNwkManagerBitMask(gaServerMask) || pMessageComingIn->Status != gZbSuccess_c){
    return;
  }
...
)
{
  ...
  */
  channelList = *((uint32_t *)gaScanChannelList);
  ...
  */
  ...
  if(mpFA_ListOfRouters != NULL){
    numberOfRouters = mpFA_ListOfRouters->cDestAddrListLength;
  }
...
*  ------  ------  --------
*  180208    BN    Created
*  220812    GTC   Energy Scan requests are sent on a timer to avoid saturation
*                  of the APS Tx state machine(see gApsMaxDataHandlingCapacity_c).
************************************************************************************/
...
)
{
  zbChannels_t channelList;
    ...
    one. */
  if( !mpFA_ListOfRouters ) {
      // Allocate memory for the list of destination network addresses.
    mpFA_ListOfRouters = MSG_Alloc(sizeof(nwkAddrList_t) + (gMinNumberOfRouters_c - 1) * sizeof(zbNwkAddr_t));
    ...
    mpFA_ListOfRouters->iCurrentRouter = 0;
      // Populate the list of destination addresses.
    mpFA_ListOfRouters->cDestAddrListLength = FA_GetRoutersForScanningChannels( mpFA_ListOfRouters->aDestAddrList, gMinNumberOfRouters_c );
  }
  
    /* Send the Energy Scan request to the current router. */
  if(mpFA_ListOfRouters->iCurrentRouter < mpFA_ListOfRouters->cDestAddrListLength) {
    *((uint32_t *)channelList) = Native2OTA32(*((uint32_t *)gaScanChannelList));
      // Send the energy request to the current router
    ASL_Mgmt_NWK_Update_req(mpFA_ListOfRouters->aDestAddrList[mpFA_ListOfRouters->iCurrentRouter],
                            channelList,
                            gDefaultScanDuration_c,
                            gDefaultScanCount_c);
      // Increment the current router index in the mpFA_ListOfRouters
    mpFA_ListOfRouters->iCurrentRouter++;
  ...
  if(mpFA_ListOfRouters->iCurrentRouter >= mpFA_ListOfRouters->cDestAddrListLength) {
    ...
    if(mpFA_ListOfRouters != NULL)
    {
      MSG_Free(mpFA_ListOfRouters);
      mpFA_ListOfRouters = NULL;
    }
  }
    /* If there are still routers to send the Energy Scan request start the timer
    for the next request. */
  else {
    TMR_StartSingleShotTimer(gFA_ReqEnergyScanToRoutersTimer, gApsAckWaitDuration_c * (gApsMaxRetries_c + 1), FA_TimeoutHandler);
  }
}
#endif
...
)
{
  ...
  FLib_MemCpy(&channelMask, ApsmeGetRequest(gApsChannelMask_c), sizeof(uint32_t));
  channelMask = OTA2Native32(channelMask);
  channelMask = (channelMask >> 11);
...
{
  if(gFATaskId != gTsInvalidTaskID_c)
  {
    /*
  ...
  */
  gFA_ReqEnergyScanToRoutersTimer = TMR_AllocateTimer();
#endif
...
)
{
  ...
  */
  ...
  {
    /*
      The scan was requested through a Mgmt_NWK_Update_request, send the scan
      results no matter what the energy values are
    */
    sendUpdate = TRUE;
  }
  ...
  */
  ...
  {
    /*
    If this energy scan does not indicate higher energy on the current channel
    then other channels, no action is taken.
    */
    for(i=0; i<16; i++)
    {
      if (!((channelMask >> i) & 1))
      ...
      if(pScanCnf->resList.pEnergyDetectList[i] < pScanCnf->resList.pEnergyDetectList[NlmeGetRequest(gNwkLogicalChannel_c) - 11])
      {
        sendUpdate = TRUE;
      }
    }
  ...
  *((uint32_t *)channelList) = Native2OTA32(*((uint32_t *)gaScanChannelList));
  ...
  /*
    If there are more energy scans left to do, process the next one, but not too
    often to avoid memory and delivery problems.
  */
  if (gScanCount)
  {
  ...
  else if(sendUpdate)
  {
    ...
    */
    TMR_StartMinuteTimer(gFA_NotifyTimer, gTimeBeforeNextNwkUpdateNotify_c, NULL);
    ResetTxCounters();
  }
#endif
}
...
* described in 053474r20 ZigBee Pro spec Annex E, uses a task and events. This state
* machine is also used by the routers particiapting in the FA procedure.
*
* Interface assumptions:
*   NONE.
*
      ...
      if ((NlmeGetRequest(gDevType_c) != gEndDevice_c)&& ZdoGetNwkManagerBitMask(gaServerMask))
        ...
        if (ZdoGetNwkManagerBitMask(gaServerMask)){
          ...
          *((uint32_t *)aChannels) = FA_BuildListFromChannel(NlmeGetRequest(gNwkLogicalChannel_c));
          ASL_Mgmt_NWK_Update_req((uint8_t *)gaBroadcastRxOnIdle, aChannels, 0xfe, NlmeGetRequest(gNwkUpdateId_c));
        }
#endif
        TMR_StartSingleShotTimer(mGenericFATimer,
                                 NlmeGetRequest(gNwkNetworkBroadcastDeliveryTime_c),
...
void NwkStartOrStopNwkConcentratorDiscoveryTime(bool_t startOrStop)
{
  ...
  if( !startOrStop )
  {
    TMR_StopSecondTimer(gNwkConcentratorDiscoveryTimerID);
  }
  else
  {
    /* Otherwise, start again the timer */
    TMR_StartSecondTimer(gNwkConcentratorDiscoveryTimerID, NlmeGetRequest(gNwkConcentratorDiscoveryTime_c), ZDO_StateMachineTimerCallBack);
  }
} 
#endif
...
void AddrMap_InitTable(void)
{
  ...
  /* Clear address map */
  BeeUtilLargeZeroMemory(gaApsAddressMap, 
  ...
  {
    Set2Bytes(gaApsAddressMap[i].aNwkAddr, 0xFFFF);
  }
  
  /* Clear the gaApsAddressMapBitMask */
  BeeUtilZeroMemory(gaApsAddressMapBitMask, gApsAddressMapBitMaskInBits_c);
}
...
{
  ZdoNwkMng_SaveToNvm(iNvmObject, &gpAddressMapTable[index]);
}
...
{
  ZdoNwkMng_SaveToNvm(iNvmObject, &gaApsDeviceKeyPairSet[index]);
}
...
/*
  GetMaxApplicationPayloadByAddrInfo

  Returns the maximum application payload (ASDU) given the address information.  
*/
