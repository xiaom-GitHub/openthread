  ...
  switch(clusterId)
  {
    ...
    case gNode_Desc_req_c:
      if (IsSelfNwkAddress(((zbNodeDescriptorRequest_t *)pMessageComingIn)->aNwkAddrOfInterest))
      {
        ...
        Copy2Bytes(pNodeDescriptor->aMaxOutTransferSize, gBeeStackConfig.aMaxOutTransferSize);
...
)
{
  ...
  */
  ...
  {
    /* Even if there is no entry to return, the status must be success (2.4.4.3.4 053474r20 ZigBee Pro spec) */
    return (0);
  }
...
)
{
	...
	for (cIndex = startIndex; cIndex < gMaximumBindingCacheTableList_c; cIndex++)
	{
		pPtr = &gaBindingCacheTable[cIndex];
...
)
{
  ...
  */
  ...
  if ((gIsABroadcast) && (status != gZbSuccess_c))
    return;
