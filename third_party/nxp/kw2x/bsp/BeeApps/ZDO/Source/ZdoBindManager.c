...
bool_t mEdbError;
zdoEdbTicksLeft_t mEdbTicksLeft;            /* time to wait for the 2nd end-device-bind req */
zdoEdbTicksLeft_t mEdbProcessTicksLeft;     /* time to wait for the end device bind process
                                               to complete */
...
zbSize_t ZdpEndDeviceBindGotRequest(zbEndDeviceBindRequest_t *pReqFrame)
{
  ...
  else if(!mEdbError && gZdoInEndDeviceBind < mEdbMaxEntries_c)
  {
    ...
    if(gZdoInEndDeviceBind == mEdbMaxEntries_c)
    {
      /* All the EndDeviceBind.Request were received */
      mEdbTicksLeft = 0;
      mEdbProcessTicksLeft = mEdbProcessTicksToExpire_c;
    }
  }
...
bool_t EdbSendResponse(index_t i)
{
  ...
  gZdpResponseCounter = pPtr->iTransactionId;
