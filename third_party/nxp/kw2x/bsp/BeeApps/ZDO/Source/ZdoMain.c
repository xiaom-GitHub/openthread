...
void ZdoReset( void )
{
  ...
  MSG_FreeQueue(&mNlmeZdoInputQueue);  /* NLME_ZDO_SapHandler */
  MSG_FreeQueue(&mAppZdpInputQueue);   /* APP_ZDP_SapHandler */
...
#if gStandardSecurity_d || gHighSecurity_d
  MSG_FreeQueue(&mApsmeZdoInputQueue);
#if gTrustCenter_d || gComboDeviceCapability_d
  MSG_FreeQueue(&gJoinIndicationQueue);
  MSG_FreeQueue(&gUpdateDeviceQueue);
#endif
#endif
    ...
    case gWaitForProcessingState_c:
      ...
      */
...
#if gApsLinkKeySecurity_d
              if (!SSP_ApsIsLinkEstablished(pUpdateMsg->aDeviceAddress))
              {
      ...
      if (events & gDeviceForbiden_c)
      {
        ...
        if (pUpdateMsg)
        {
          ZDO_APSME_Remove_Device_request(pUpdateMsg->aSrcAddress, pUpdateMsg->aDeviceAddress);
        }
#endif        
        /*
          If the authentication fails or not, we are done with this packet.
