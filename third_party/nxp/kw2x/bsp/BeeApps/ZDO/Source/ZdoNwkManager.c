...
void RemoveNextHopRouteEntry(uint8_t *pShortAddress);
#endif
#endif
...
/*
  This function only helps to hold the saving common code into one place instead of repeating
...
void ZdoNwkMng_SaveToNvm(zdoNvmObject_t iNvmObject, void* pAddress)
{
  switch(iNvmObject)
  {
    ...
    case zdoNvmObject_NwkData_c:
      ZdoNwkMng_SaveDataSetPtr(&gNwkData, gNvSaveOnInterval_c, TRUE);
      break;
    case zdoNvmObject_NeighborTable_c:
      ...
      {
        ZdoNwkMng_SaveDataSetPtr(&gaNeighborTable, gNvSaveOnInterval_c, TRUE);
      }
      break;
   ...
   //   break;
    ...
    case zdoNvmObject_AddrMapPermanent_c:
      ...
      {
        ZdoNwkMng_SaveDataSetPtr(pAddress, gNvSaveOnIdle_c, FALSE);
        ZdoNwkMng_SaveDataSetPtr(gaApsAddressMapBitMask, gNvSaveOnIdle_c, TRUE);
      }
      break;
    ...
    case zdoNvmObject_BindingTable_c:
      if(pAddress)
      {
        if((pAddress >= (void*)gaApsBindingTable)
           && (pAddress <= (void*)&gaApsBindingTable[gMaximumApsBindingTableEntries_c]))
        {
          ZdoNwkMng_SaveDataSetPtr(pAddress, gNvSaveOnIdle_c, FALSE);
        }
      }
      ...
      {
        ZdoNwkMng_SaveDataSetPtr(&gaApsBindingTable, gNvSaveOnInterval_c, TRUE);
      }
      break;
    ...
    case zdoNvmObject_GroupTable_c:
      ...
      {
        ZdoNwkMng_SaveDataSetPtr(&gaApsGroupTable, gNvSaveOnInterval_c, TRUE);
      }
      break;  
    ...
    case zdoNvmObject_ApsLinkKeySet_c:
      if(pAddress)
      {
        ...
        */
        ...
        {
          ZdoNwkMng_SaveDataSetPtr(&gaApsKeySet[((zbApsDeviceKeyPairSet_t*)pAddress)->iKey], gNvSaveOnIdle_c, FALSE);
        }
      }
      else
      {
        ZdoNwkMng_SaveDataSetPtr(&gaApsDeviceKeyPairSet, gNvSaveOnInterval_c, TRUE);
        ZdoNwkMng_SaveDataSetPtr(&gaApsKeySet, gNvSaveOnInterval_c, TRUE);
        ZdoNwkMng_SaveDataSetPtr(&gaApsAddressMap, gNvSaveOnInterval_c, TRUE);
        ZdoNwkMng_SaveDataSetPtr(&gaApsAddressMapBitMask, gNvSaveOnInterval_c, TRUE);
      }
      break;
#endif
    ...
    case zdoNvmObject_NwkOutgoingSecCounters_c:
    {
      ...
      {
        if(gNwkData.aNwkSecurityMaterialSet[i].keySeqNumber == NlmeGetRequest(gNwkActiveKeySeqNumber_c))
        {
          ZdoNwkMng_SaveDataSetPtr(&(gNwkData.aNwkSecurityMaterialSet[i].outgoingFrameCounter), gNvSaveOnCount_c, TRUE);
        }
      }
      break;
    }
      ...
      else
      {
        ...
        for(i=0; i< NumberOfElements(NlmeGetRequest(gNwkSecurityMaterialSet_c)); i++)
        {
          if(gNwkData.aNwkSecurityMaterialSet[i].keySeqNumber == NlmeGetRequest(gNwkActiveKeySeqNumber_c))
          {
            if(i == 0)
            {
              ZdoNwkMng_SaveDataSetPtr(&gaIncomingFrameCounterSet1, gNvSaveOnInterval_c, TRUE);
            }
            else if(i == 1)
            {
              ZdoNwkMng_SaveDataSetPtr(&gaIncomingFrameCounterSet2, gNvSaveOnInterval_c, TRUE);
            }
          }
        }
      }
